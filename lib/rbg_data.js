...
class RBG_DocPage_C {
  ...
  Render(sCanvasID,oRenderOptions)
  {
    ...
    if (aElements) {
      ...
      ctx.translate(nXOffset, nYOffset);
      // Render each element
      for (var i = 0; i < aElements.Elements.length; i++) {
        ...
        switch (e.type) {
          case 'text':
            ...
            ctx.shadowOffsetX = e.ShadowOffsetX;
            ctx.shadowOffsetY = e.ShadowOffsetY;
            if (e.Rotation.Value != 0)
              ctx.rotate(e.Rotation.toUnit('rad'));
            ctx.fillText(e.Text, e.X.toUnit('px',96.0), e.Y.toUnit('px',96.0));
            ctx.restore();
            break;
          case 'rect':
            ctx.save();
          ...
          case 'image':
            var image = new Image();
            image.onload = this.DrawImageCallback.bind(this,image,ctx,e);
            image.src = e.Filename;
            break;
        }
      }
    } else {
  ...
  DrawImageCallback(image,ctx,e)
  {
    ctx.save();
    if (e.Width.Value > 0)
      ctx.drawImage(image, e.X.toUnit('px',96.0), e.Y.toUnit('px',96.0),
                           e.Width.toUnit('px',96.0), e.Height.toUnit('px',96.0));
    else
      ctx.drawImage(image, e.X.toUnit('px',96.0), e.Y.toUnit('px',96.0))
    ctx.restore();
